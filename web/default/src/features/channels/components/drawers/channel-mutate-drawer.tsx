...
import { W3OAuthDialog } from '../dialogs/w3-oauth-dialog'
...
export function ChannelMutateDrawer({
  ...
  const [w3OAuthDialogOpen, setW3OAuthDialogOpen] = useState(false)
  const [isCodexCredentialRefreshing, setIsCodexCredentialRefreshing] =
    useState(false)
  const [isW3CredentialRefreshing, setIsW3CredentialRefreshing] =
    useState(false)
  ...
  const w3OAuthEnabled = form.watch('w3_oauth_enabled')
  ...
  const handleRefreshW3Credential = useCallback(async () => {
    if (!channelId) return
    setIsW3CredentialRefreshing(true)
    try {
      const res = await refreshW3Credential(channelId)
      if (!res.success) {
        throw new Error(res.message || 'Failed to refresh credential')
      }
      toast.success(t('Credential refreshed'))
      ...
      setIsW3CredentialRefreshing(false)
    }
  }, [channelId, queryClient, t])
